/*
 * Support js for Entities_micromarket
 */

function Entities_micromarket(uri_) {
    this.uri = uri_;
}

Entities_micromarket.prototype = {
    getUri: function() {
        return this.uri;
    },
    ...
    remove: function(uri_) {
        ...
        var remote = new Entities_micromarketRemote(this.uri);
    ...
    /* Method findAll with HTTP request metod GET, its return type is array of MicroMarket  */
        ...
        for (var prop in myObj) {
            ...
            for (j = 0; j < ref.length; j++) {
                if (ref[j]['@uri'] != null && ref[j]['@uri'] != undefined) {
                    result[j] = new MicroMarket(ref[j], ref[j]['@uri']);
                }
                else {
                    result[j] = new MicroMarket(ref[j]);
                }
            }
        }
        return result;
    },
    /* Method edit with HTTP request metod PUT and path : {id}, its parameter type is MicroMarket  */
    ...
    /* Method create with HTTP request metod POST, its parameter type is MicroMarket  */
    ...
    /* Method findRange with HTTP request metod GET and path : {from}/{to}, its return type is array of MicroMarket  */
    ...
    /* Method find with HTTP request metod GET and path : {id}, its return type is MicroMarket  */
    find: function(uri_) {
        var url = "";
        if (uri_ != null && uri_ != undefined) {
            url = uri_;
        ...
        if (myObj['@uri'] != null && myObj['@uri'] != undefined) {
            return new MicroMarket(myObj, myObj['@uri']);
        }
        else {
            return new MicroMarket(myObj);
        }
    },
    toString: function() {
        var myObj =
...
function Entities_micromarketRemote(uri_) {
    this.uri = uri_;
}

Entities_micromarketRemote.prototype = {
