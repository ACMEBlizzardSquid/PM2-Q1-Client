/*
 * Support js for Entities_discountcode
 */

function Entities_discountcode(uri_) {
    this.uri = uri_;
}

Entities_discountcode.prototype = {
    getUri: function() {
        return this.uri;
    },
    ...
    remove: function(uri_) {
        ...
        var remote = new Entities_discountcodeRemote(this.uri);
    ...
    /* Method findAll with HTTP request metod GET, its return type is array of DiscountCode  */
    findAll: function(uri_) {
        ...
        for (var prop in myObj) {
            var ref = myObj[prop];
            result.push(new DiscountCode(ref, this.uri + ref.discountCode))
        }
        return result;
    },
    /* Method edit with HTTP request metod PUT and path : {id}, its parameter type is DiscountCode  */
    ...
    /* Method create with HTTP request metod POST, its parameter type is DiscountCode  */
    ...
    /* Method findRange with HTTP request metod GET and path : {from}/{to}, its return type is array of DiscountCode  */
        ...
        for (var prop in myObj) {
            ...
            for (j = 0; j < ref.length; j++) {
                if (ref[j]['@uri'] != null && ref[j]['@uri'] != undefined) {
                    result[j] = new DiscountCode(ref[j], ref[j]['@uri']);
                }
                else {
                    result[j] = new DiscountCode(ref[j]);
                }
            }
        }
        return result;
    },
    /* Method find with HTTP request metod GET and path : {id}, its return type is DiscountCode  */
    find: function(uri_) {
        var url = "";
        if (uri_ != null && uri_ != undefined) {
            url = uri_;
        ...
        if (myObj['@uri'] != null && myObj['@uri'] != undefined) {
            return new DiscountCode(myObj, myObj['@uri']);
        }
        else {
            return new DiscountCode(myObj);
        }
    },
    toString: function() {
        var myObj =
...
function Entities_discountcodeRemote(uri_) {
    this.uri = uri_;
}

Entities_discountcodeRemote.prototype = {
